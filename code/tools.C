...
int tools::addFiles(TChain *c, string filelistname) {
    ...
    if (ibad != 0)
        cerr << "Warning: failed to load " << ibad << " files  out of " << igood
             << endl;
...
// Ratio of two graphs
void tools::GetPoint(TGraphErrors *g, int n, double &x, double &y, double &ex,
                     double &ey) {
    g->GetPoint(n, x, y);
    ex = g->GetErrorX(n);
    ey = g->GetErrorY(n);
}
void tools::SetPoint(TGraphErrors *g, int n, double x, double y, double ex,
                     double ey) {
    g->SetPoint(n, x, y);
...
TGraphErrors *tools::ratioGraphs(TGraphErrors *g1, TGraphErrors *g2,
    ...
    for (int i = 0, j = 0; (i != g1->GetN() && j != g2->GetN());) {
        ...
        if (fabs(x1 - x2) <= fabs(x1 - x2m) &&
            fabs(x1 - x2) <= fabs(x1 - x2p) &&
            fabs(x1 - x2) <= fabs(x2 - x1m) &&
            fabs(x1 - x2) <= fabs(x2 - x1p)) {
            int n = g->GetN();
            SetPoint(g, n, 0.5 * (x1 + x2), (y2 ? y1 / y2 : 0.),
                     0.5 * fabs(x1 - x2), oplus(ey1 / y1, ey2 / y2) *
                                              fabs(y2 ? y1 / y2 : 0.) * erry);
            ++i, ++j;
        } else
            (x1 < x2 ? ++i : ++j);
    } // for i, j
...
TH1D *tools::Divide(const TH1D *h1, const TH1D *h2, double c1, double c2,
    ...
    TH1D *h3 =
        (TH1D *)h1->Clone(Form("ratio_%s_%s", h1->GetName(), h2->GetName()));
...
TH1D *tools::Rebin(const TH1D *h, const TH1D *href) {
    ...
    if (!(href->GetNbinsX() <= h->GetNbinsX())) {
        cout << "Histo has less bins than ref: " << h->GetNbinsX() << " vs "
             << href->GetNbinsX() << " for " << h->GetName() << endl;
    }
...
void tools::Hadd(TH1 *h1, TH1 *h2, double ptmax, bool syserr) {
    ...
    for (int i = 1; i != h1->GetNbinsX() + 1; ++i) {
        ...
        if (j > 0 && j < h2->GetNbinsX() + 1 &&
            ...
            if (syserr)
                ey = (n1 + n2 ? (ey1 * n1 + ey2 * n2) / (n1 + n2) : 0.);
